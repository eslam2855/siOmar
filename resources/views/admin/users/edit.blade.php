    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center">
                <h1 class="text-2xl font-semibold text-gray-900">{{ __('admin.edit_user') }}</h1>
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <div>
                                <label for="password" class="block text-sm font-medium text-gray-700 mb-2">
                                    <i class="fas fa-lock mr-2 text-indigo-500"></i>{{ __('admin.password') }} ({{ __('admin.leave_blank_keep_current') }})
                                </label>
                            ...
                            <div>
                                ...
                                <select id="role" name="role" required 
                                    ...
                                    <option value="user" {{ old('role', $user->roles->first()->name ?? '') == 'user' ? 'selected' : '' }}>{{ __('admin.user') }}</option>
                                    <option value="admin" {{ old('role', $user->roles->first()->name ?? '') == 'admin' ? 'selected' : '' }}>{{ __('admin.admin') }}</option>
                                </select>
                                @error('role')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
