    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-bold text-gray-900">{{ __('admin.edit_slider') }}</h2>
            ...
            <form method="POST" action="{{ route('admin.sliders.update', $slider) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    @if($slider->image)
                        <div class="mb-4">
                            <p class="text-sm text-gray-600 mb-2">{{ __('admin.current_image') }}:</p>
                    ...
                    <input type="file" name="image" id="image" accept="image/*"
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    <p class="mt-1 text-sm text-gray-500">{{ __('admin.image_requirements') }}. {{ __('admin.leave_empty_keep_current') }}</p>
                    @error('image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="order" id="order" value="{{ old('order', $slider->order) }}" min="0"
                           class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    <p class="mt-1 text-sm text-gray-500">{{ __('admin.lower_numbers_first') }}</p>
                    @error('order')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit" 
                        ...
                        {{ __('admin.update') }} {{ __('admin.slider') }}
                    </button>
                </div>
            </form>
        </div>
